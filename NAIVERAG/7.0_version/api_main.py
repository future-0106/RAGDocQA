"""FastAPI版多PDF知识库系统：支持HTTP接口访问 + HTML页面托管"""
import warnings
import os
import json
import torch
...
app = FastAPI(
    title="多PDF智能检索API",
    description="支持PDF上传、查看、全局检索回答的API服务 + HTML页面托管",
    version="1.0.0"
)

# ========== 配置静态文件和HTML模板 ==========
# 创建所需目录（若不存在）
os.makedirs("templates", exist_ok=True)  # 存放HTML文件
os.makedirs("static", exist_ok=True)     # 存放CSS/JS/图片等静态资源
os.makedirs("./temp_uploads", exist_ok=True)  # 临时上传文件目录

# 配置模板引擎（Jinja2）
templates = Jinja2Templates(directory="templates")
# 挂载静态文件目录（前端可通过/static/xxx.css访问）
...
# ========== HTML页面接口 ==========
@app.get("/page", summary="前端HTML页面")
async def get_html_page(request: Request):
    """返回带PDF检索功能的前端HTML页面"""
    return templates.TemplateResponse(
        "index.html",  # 对应templates/index.html文件
        {
            "request": request,
...
async def home():
    ...
    return JSONResponse({
        ...
        "web_page_url": "http://127.0.0.1:8000/page",  # 新增HTML页面入口
        "available_models": list(ALL_MODELS.keys()),
        "default_model": DEFAULT_MODEL,
        "endpoints": {
...
if __name__ == "__main__":
    ...
    print(f"🌐 前端页面：http://127.0.0.1:8000/page")
    ...
    uvicorn.run(
        ...
        host="0.0.0.0",
        port=8000,
        reload=False  # 开发环境可改为True，修改HTML/代码自动重启
    )