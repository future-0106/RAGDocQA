...
try:
    ...
    print("✅ PaddleOCR初始化成功（禁用OneDNN+CPU模式）")
...
def check_cache_valid(pdf_path: str) -> tuple[bool, str]:
    ...
    if cache_meta.get("modify_time") == current_modify_time:
...
def clean_expired_cache(days: int = 7):
    ...
    for content_hash in os.listdir(CACHE_DIR):
        ...
        try:
            ...
            print(f"⚠️ 清理缓存失败 {content_hash[:8]}...：{e}")
    ...
    for content_hash in os.listdir(CACHE_DIR):
        ...
        if os.path.exists(meta_path):
            try:
                ...
                print(f"- 内容哈希：{content_hash[:8]}... | 缓存元数据损坏：{e}")

# ========== 新增：已上传PDF记录管理 ==========
def init_upload_record():
    """初始化上传记录文件（不存在则创建）"""
...
def is_pdf_uploaded(content_hash: str) -> bool:
    """判断PDF是否已上传（基于内容哈希）"""
    uploaded_list = get_uploaded_pdfs()
    return any(item["content_hash"] == content_hash for item in uploaded_list)

def add_upload_record(pdf_path: str, content_hash: str):
    """添加PDF上传记录"""
    from config import UPLOAD_RECORD_FILE
    init_upload_record()
    file_stat = os.stat(pdf_path)
    ...
    # 先读取现有记录，去重后追加
    uploaded_list = get_uploaded_pdfs()
    uploaded_list = [item for item in uploaded_list if item["content_hash"] != content_hash]
    uploaded_list.append(record)
    # 保存回文件
    with open(UPLOAD_RECORD_FILE, "w", encoding="utf-8") as f:
        json.dump(uploaded_list, f, ensure_ascii=False, indent=2)

def list_uploaded_pdfs_detail():
    """格式化输出已上传PDF列表"""
    uploaded_list = get_uploaded_pdfs()
    ...
    print("\n📋 已上传PDF列表：")
    print("-" * 100)
    print(f"{'序号':<4} {'文件名':<30} {'文件大小(KB)':<12} {'上传时间':<20} {'内容哈希(前8位)':<10}")
    print("-" * 100)
    for idx, item in enumerate(uploaded_list, 1):
        file_size_kb = round(item["file_size"] / 1024, 2)
        print(f"{idx:<4} {item['pdf_name']:<30} {file_size_kb:<12} {item['upload_time']:<20} {item['content_hash'][:8]:<10}")
    print("-" * 100)
