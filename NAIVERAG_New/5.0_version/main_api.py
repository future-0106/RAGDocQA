    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "models": {
                ...
                "llm_params": self.llm._identifying_params
            },
            "device": str(DEVICE),
            "config": {
                "chunk_size": CHUNK_SIZE,
...
class QueryRequest(BaseModel):
    ...
    model_key: Optional[str] = Field(default=None, description="要使用的模型键值，不指定则使用当前模型")
...
async def query_rag(request: QueryRequest):
    ...
    try:
        ...
        rag_system.query_history.append({
            ...
            "timestamp": datetime.now().isoformat(),
            "model": request.model_key if request.model_key else rag_system.llm.model_key
        })
        ...
        # 确定要使用的模型
        use_model_key = request.model_key if request.model_key else rag_system.llm.model_key

        # 如果指定的模型与当前模型不同，创建新的RAG流水线
        if use_model_key != rag_system.llm.model_key:
            try:
                # 创建新的LLM实例（使用指定模型）
                temp_llm = MultiModelLLM(model_key=use_model_key)

                # 创建新的RAG流水线
                temp_rag_pipeline = QwenRAGPipeline(temp_llm, rag_system.vector_manager)

                # 使用临时流水线执行查询
                result = temp_rag_pipeline.query(
                    request.question,
                    k=request.k,
                    score_threshold=request.score_threshold
                )

                used_model = use_model_key

            except Exception as e:
                print(f"❌ 使用指定模型 {use_model_key} 失败: {e}")
                # 如果指定模型失败，使用默认模型
                ...
                used_model = rag_system.llm.model_key
        else:
            # 使用默认模型
            result = rag_system.rag_pipeline.query(
                request.question,
