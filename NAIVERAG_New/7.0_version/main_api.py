...
class RAGAPISystem:
    ...
    def _init_components(self):
        ...
        try:
            ...
            # 8. 初始化文件管理器（传入hybrid_manager以便自动更新BM25）
            print("🔧 初始化文件管理器...")
            self.file_manager = FileVectorizationManager(data_dir=DATA_DIR, hybrid_manager=self.hybrid_manager)
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        success, message, documents = rag_system.file_manager.upload_and_vectorize(
            ...
            rag_system.hybrid_manager   # 新增参数，确保BM25索引同步更新
        )
...
async def batch_upload(request: BatchUploadRequest):
    ...
    try:
        ...
        results = rag_system.file_manager.upload_multiple_files(
            ...
            rag_system.document_processor,
            rag_system.vector_manager,
            rag_system.hybrid_manager   # 新增参数
        )
...
async def switch_model(request: ModelSwitchRequest):
    ...
    try:
        ...
        # 重新初始化RAG流水线
        rag_system.rag_pipeline = QwenRAGPipeline(rag_system.llm, rag_system.hybrid_manager)
...
async def switch_embedding(request: ModelSwitchRequest):
    ...
    try:
        ...
        # 构造函数会自动调用 _load_documents_to_bm25()，加载现有文档到BM25索引

        # 4. 更新文件管理器中的hybrid_manager引用
...
async def reprocess_files():
    ...
    try:
        ...
        if all_document_texts:
            ...
            print(f"📊 已更新BM25索引，共 {len(all_document_texts)} 个文档块")
