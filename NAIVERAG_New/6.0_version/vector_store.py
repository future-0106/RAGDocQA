...
# FileVectorizationManager 类保持不变，但需要修改以支持混合检索
class FileVectorizationManager:
    ...
    def upload_and_vectorize(self, source_path: str, processor, vector_manager, hybrid_manager=None) -> Tuple[
        bool, str, List]:
        """上传文件并立即转换为向量，一步到位"""
        ...
        try:
            ...
            if hybrid_manager and document_texts:
                hybrid_manager.add_documents(document_texts)
    ...
    def upload_multiple_files(self, source_paths: List[str], processor, vector_manager, hybrid_manager=None) -> Dict[
        str, Any]:
        """批量上传文件并转换为向量"""
        results = {
            "success": [],
