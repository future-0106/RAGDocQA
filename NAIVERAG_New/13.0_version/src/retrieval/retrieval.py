        ...
        try:
            ...
            for idx, rerank_score in reranked_indices:
                if idx < len(initial_results):
                    ...
                    # 融合分数：0.6 * 重排分数 + 0.4 * 原始相似度（需归一化到相似范围）
                    # 重排分数通常是原始logits，范围不定，此处简单加权
                    fused_score = 0.6 * rerank_score + 0.4 * original_score
                    reranked_results.append((doc_text, fused_score))
