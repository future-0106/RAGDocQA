...
def check_imports():
    ...
    try:
        ...
        print("[OK] Using langchain_huggingface.HuggingFaceEmbeddings")
    except ImportError:
        try:
            from langchain_community.embeddings import HuggingFaceEmbeddings
            print("[WARNING] Using langchain_community.HuggingFaceEmbeddings")
        except ImportError:
            print("[ERROR] Cannot import HuggingFaceEmbeddings")
            print("Please install: pip install langchain-huggingface or langchain-community")
            return False

    try:
        from langchain_chroma import Chroma
        print("[OK] Using langchain_chroma.Chroma")
    except ImportError as e:
        print(f"[ERROR] Cannot import ChromaDB: {e}")
        print("Please install: pip install chromadb langchain-chroma")
        return False

    try:
        import jieba
        print("[OK] Using jieba")
    except ImportError:
        print("[ERROR] Cannot import jieba")
        print("Please install: pip install jieba")
        return False

    try:
        from rank_bm25 import BM25Okapi
        print("[OK] Using BM25 retrieval")
    except ImportError:
        print("[ERROR] Cannot import BM25")
        print("Please install: pip install rank-bm25")
        return False
