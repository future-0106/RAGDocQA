...
# 导入现有模块
from config import *
from core.models import MultiModelLLM, MultiEmbeddings, ModelFactory
...
from retrieval.rag_pipeline import QwenRAGPipeline, setup_environment, check_imports
from core.models import MultiReranker
        ...
        try:
            ...
            if QUERY_REWRITING_MODEL:
                try:
                    ...
                    print(f"[WARNING] Rewrite model failed, using QA model: {e}")
            ...
            self.agent_coordinator = AgentCoordinator(self.llm, use_external_api=True)
            # ============================================
...
class ProcessDocumentsRequest(BaseModel):
    """批量处理文档请求模型"""
    source_dir: str = Field(..., description="文档目录路径")
...
async def root():
    return FileResponse(rag_system.static_dir / "index.html")
    ...
    try:
        ...
        return JSONResponse(content={
            ...
            "components": {
                ...
                "agent_coordinator": "active" if rag_system.agent_coordinator else "inactive"
            }
        })
    except Exception as e:
        return JSONResponse(
...
async def contract_review(request: ContractReviewRequest):
    ...
    try:
        ...
        return JSONResponse(content={
            "success": result.get("success", False),
            "type": result.get("type"),
            "review": result.get("review", result.get("error", "")),
            "agent": result.get("agent")
        })
    except Exception as e:
        return JSONResponse(status_code=500, content={
...
async def generate_document(request: DocumentRequest):
    ...
    try:
        ...
        return JSONResponse(content={
            "success": result.get("success", False),
            "type": result.get("type"),
            "document": result.get("document", result.get("error", "")),
...
async def risk_assessment(request: RiskAssessmentRequest):
    ...
    try:
        ...
        return JSONResponse(content={
            "success": result.get("success", False),
            "type": result.get("type"),
            "assessment": result.get("assessment", result.get("error", "")),
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"[WARNING] Cannot get full status: {e}")
